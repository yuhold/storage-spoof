...
- 主界面重构为“主页 / 应用 / 设置”三页底部导航，并补充模块状态、配置统计、安全边界和快捷操作。
- 自定义界面与 Material 3 改为两套真正独立的视觉布局；应用卡片、设置卡片、底部导航和分组标签支持莫奈动态取色或自定义种子色。
- 权限检查页与关于页同步支持双界面、明暗模式、动态主题色、自定义背景图和背景明亮度。
- 新增应用列表筛选、默认顺序与名称 A–Z 排序、显示系统应用；所有排序始终保持已修改应用在上。
- 优化圆角、背景图自适应、最低可读性遮罩、大字体显示、两行容量摘要和可滚动筛选面板。
- 保持 Android 16、libxposed API 102、AOSP 设置与小米澎湃 OS 3 安全中心兼容；未配置或停用应用继续显示真实统计。
...
- 从旧的 Debug/非官方签名构建切换到官方 Release 时，Android 可能要求先卸载旧版（这会清除模块配置）；安装官方版后，后续官方版本可正常覆盖升级。
